...
path = "/Users/silver/Dropbox/sci/pro_exo/models/rdw/"
os.chdir(path)

# Select which files
files = sorted(glob('*.dat'))
files = [ x for x in files if 'fit_par' not in x ]
cols = ['g', 'k', 'y', 'm', 'r', 'b', 'y', 'y', 'y', 'y']
zord = [ 10,  20 ,  24 ,  4,  8, 7]
XMIN = -200.
...
for ii, ff in enumerate(files):
    ...
    par = np.loadtxt(ff[:-8] + "_fit_par_rdw.dat")
...
plt.xlim([XMIN, 0])
plt.ylim([1e-1, 30])
fig.savefig('rdw.pdf', bbox_inches='tight', pad_inches=0.03)
