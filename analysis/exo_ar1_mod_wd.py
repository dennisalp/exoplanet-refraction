...
from exo_trans_refrac_wd import refraction_transit
...
Rstar = 9e8 # Radius of the star
Mstar = 0.6*1.989e33 # Mass of the star
g1 = 0.0638 # Fiducial parameters close to M4V dwarf
g2 = 0.2648
...
RR = 7.1e+9 # Radius
Dls = np.sqrt(0.015)*5.2*AU # Orbital distance
tt=np.linspace(0., 8*60., 8*60+1) # Time range [min]
...
#Dls = np.sqrt(0.015)*AU # Orbital distance
...
#Dls = np.sqrt(0.015)*(80/365.26)**(2/3.)*AU # Orbital distance
...
#MM = 1.898e30 # Mass of planet, 1.898e30 g = 1 M_Jupiter
#RR = 7.1e+9 # Radius, 7.1e+9 cm = 1 R_Jupiter. ASSUMING THIS TO BE SAME AS FOR JUPITER, I.E. AVERAGE DENSITY INDEPENDENT OF SURFACE TEMPERATURE
#Dls = np.sqrt(0.015)*(20/365.26)**(2/3.)*AU # Orbital distance
...
#np.savetxt("C:\\Users\\dalp\\Dropbox\\sci\\pro_exo\\models\\wd\\" + ID + "_wd.dat", output)
output_pars = np.c_[fix_par(guess), fix_ex_mod_par, ldc_ex_mod_par, fix_conv_mod_par, ldc_conv_mod_par].T
#np.savetxt("C:\\Users\\dalp\\Dropbox\\sci\\pro_exo\\models\\wd\\" + ID + "_fit_par_wd.dat", output_pars)
#np.savetxt('/Users/silver/Dropbox/sci/pro_exo/models/jovian_wd.dat', output)

MAC = False
if MAC:
    np.savetxt('/Users/silver/Dropbox/sci/pro_exo/models/wd/' + ID + '_wd.dat', output)
    np.savetxt('/Users/silver/Dropbox/sci/pro_exo/models/wd/' + ID + '_fit_par_wd.dat', output_pars)
else:
    np.savetxt('C:\\Users\\dalp\\Dropbox\\sci\\pro_exo\\models\\wd\\' + ID + '_wd.dat', output)
    np.savetxt('C:\\Users\\dalp\\Dropbox\\sci\\pro_exo\\models\\wd\\' + ID + '_fit_par_wd.dat', output_pars)
